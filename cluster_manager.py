...
# CONFIGURATION
BASE_IMAGE = "/var/lib/libvirt/images/alpine-base.qcow2"
VM_DIR = "/var/lib/libvirt/images/"
TEMPLATE_XML = "/opt/cluster/template.xml"
HAPROXY_CFG = "/etc/haproxy/haproxy.cfg"
HAPROXY_BASE_CFG = "/etc/haproxy/haproxy_base.cfg"
NETWORK_NAME = "default"
LOG_FILE = "/var/log/cluster_manager.log"
...
class ClusterManager:
    ...
    def get_cpu_usage(self, domain):
        ...
        try:
            if not domain.isActive(): return 0.0
    ...
    def create_vm(self, vm_index):
        ...
        subprocess.run(["qemu-img", "create", "-f", "qcow2", "-F", "qcow2", "-b", BASE_IMAGE, new_disk], check=True)
    ...
    def _wait_for_ip(self, mac_address):
        ...
        for _ in range(30):
            for lease in network.DHCPLeases():
                if lease['mac'] == mac_address: return lease['ipaddr']
            time.sleep(1)
    ...
    def destroy_vm(self, vm_name):
        ...
        try:
            ...
            logging.erorr(f"libvirt error during destroying: {e}")
    ...
    def update_haproxy(self):
        """Rebuilds HAProxy config and reloads"""
        ...
        for vm, ip in self.active_vms.items():
            # Allows HAProxy to monitor worker health
            ...
            try:
                ...
                for vm_name in list(self.active_vms.keys()):
                    try:
                        ...
                        if state != 1:
                            logging.warning(f"[FAILURE] Machine {vm_name} not responding! (State: {state})")
                ...
                if active_count > 0:
                    ...
                    logging.info(f"[MONITORING] Average Cluster CPU: {avg_cpu:.1f}% | Active nodes: {active_count}")

                    # Check if cooldown period has passed
                    if (time.time() - self.last_action_time) > COOLDOWN_PERIOD:
                        # Scale OUT
                        if avg_cpu > CPU_HIGH_THRESHOLD and active_count < MAX_VMS:
                            logging.info(f"[SCALE OUT] Surpassed {CPU_HIGH_THRESHOLD}%. Creating new machine...")
                            # Look for free index
                            new_idx = max([int(n.split('-')[1]) for n in self.active_vms.keys()]) + 1
                            self.create_vm(new_idx)
                        
                        # Scale IN
                        elif avg_cpu < CPU_LOW_THRESHOLD and active_count > MIN_VMS:
                            logging.info(f"[SCALE IN] Fell below {CPU_LOW_THRESHOLD}%. Removing redundant machine...")
                            vm_to_remove = list(self.active_vms.keys())[-1] # Remove the newest
